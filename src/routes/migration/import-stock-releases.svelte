<script>
    ...
    function cleanData(data) {
        ...
        console.log("Cleaned", stock_releases);
    }

    function selectRow(event) {
        let index = stock_releases.findIndex((x) => x.id == event.detail.id);
        stock_releases[index].selected = !event.detail.selected;
    }
    ...
    function selectAllRows(event) {
        ...
        stock_releases = event.detail.rows;
    }
    ...
    function saveSelected() {
        let numberOfPurchaseOrders = stock_releases.filter((x) => x.selected).length;
        ...
        stock_releases
            ...
            .forEach(async (element, index) => {
                try {
                    let response = await post({
                        ...
                        data: {
                            data: {
                                ...
                                srf_no: element.srf_no.toString(),
                    ...
                    if (response.data?.id) {
                        let index = stock_releases.findIndex((x) => x.id == element.id);

                        stock_releases[index].status = "Done";
                        stock_releases[index].selected = false;
                    }
                } catch (e) {
                    let index = stock_releases.findIndex((x) => x.id == element.id);

                    stock_releases[index].status = "Error";
                    stock_releases[index].selected = false;
                    ...
                    stock_releases[index].message = message;
                ...
                if (index + 1 == numberOfPurchaseOrders) {
                    toast.push(
                        stock_releases.filter((x) => x.status == "Done").length +
                            " Stock Releases Added Successfully!",
                        {
                            duration: 20000,
                            theme: {
...
<div class="container px-6">
    ...
    <div class="card px-6">
        ...
        <div class="columns">
            <div class="column">
                <h3 class="my-0">Import Stock Releases</h3>
        ...
        {:else if stock_releases}
            <div class="has-text-centered">
                ...
                <p class="light-gray">No Stock Releases Found in the file</p>
                <br /><br /><br /><br />
            </div>
        {:else}
            <div class="has-text-centered">
