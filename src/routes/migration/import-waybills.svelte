<script>
    ...
    function cleanData(data) {
        ...
        console.log("Cleaned", waybills);
    }

    function selectRow(event) {
        let index = waybills.findIndex((x) => x.id == event.detail.id);
        waybills[index].selected = !event.detail.selected;
    }
    ...
    function saveSelected() {
        let numberOfPurchaseOrders = waybills.filter((x) => x.selected).length;
        ...
        waybills
            ...
            .forEach(async (element, index) => {
                try {
                    let response = await post({
                        ...
                        data: {
                            data: {
                                ...
                                transporterName: element.transporterName?.toString(),
                                transporterPhone: element.transporterPhone?.toString(),
                                transporterType: element.transporterType?.toString(),
                                driverName: element.driverName?.toString(),
                                driverPlateNo: element.driverPlateNo?.toString(),
                                driverTransportType: element.driverTransportType?.toString(),
                                driverPhone: element.driverPhone?.toString(),
                                consigneeName: element.consigneeName?.toString(),
                                consigneephone: element.consigneePhone?.toString(),
                                consigneeAddress: element.consigneeAddress?.toString(),
                            },
                        },
                    });
                    ...
                    if (response.data?.id) {
                        let index = waybills.findIndex((x) => x.id == element.id);

                        waybills[index].status = "Done";
                        waybills[index].selected = false;
                    }
                } catch (e) {
                    let index = waybills.findIndex((x) => x.id == element.id);

                    waybills[index].status = "Error";
                    waybills[index].selected = false;
                    ...
                    waybills[index].message = message;
                ...
                if (index + 1 == numberOfPurchaseOrders) {
                    toast.push(
                        waybills.filter((x) => x.status == "Done").length +
                            " Waybills Added Successfully!",
                        {
                            duration: 20000,
                            theme: {
...
<div class="container px-6">
    ...
    <div class="card px-6">
        ...
        <div class="columns">
            <div class="column">
                <h3 class="my-0">Import Waybills</h3>
        ...
        {:else if waybills}
            <div class="has-text-centered">
                ...
                <p class="light-gray">No Waybills Found in the file</p>
                <br /><br /><br /><br />
            </div>
        {:else}
            <div class="has-text-centered">
