...
import { exportToCsvAlternate } from "../../utils/export/csvGenerator";
    ...
    const columns = [
        ...
        {
            key: "stock_release_item",
            title: "Stock Release Item",
            sortable: true,
            filterValue: (v) => v.last_name.charAt(0).toLowerCase(),
            filterType: "string",
            selected: true,
            ...
            if (e.target.files[0].type == "text/csv") {
                ...
                let temp = utils.sheet_to_json(workbook.Sheets.Sheet1, {
                    ...
                    header: [
                        ...
                        "unitPrice",
                        "poNumber",
                        "stock_release_old_id",
                    ],
                });
                temp.forEach((element, index) => {
                    element.id = index;
                    element.waybill = waybills.filter(
                        (x) => x.attributes.old_id == parseInt(element.old_id)
                    )[0];
                    element.stock_release = stock_releases.filter(
                        (x) =>
                            x.attributes.srf_no == element.srf_no.toString()
                    )[0];
                    element.stock_release_item = element.stock_release?.attributes.stock_release_items?.data?.filter( x => x.attributes.old_id == parseInt(element.stock_release_old_id) )[0]
                });
                console.log(temp);

                exportToCsvAlternate("Overreleased Waybill Items ", temp.filter( x => x.stock_release?.length == 0 ).map( x => {
                    return [
                        x.srf_no,
                        x.poNumber,
                        x.date,
    ...
    function cleanData(data) {
        ...
        waybill_items = data.filter(
            (x) => x.waybill && x.quantity && x.stock_release && x.stock_release_item
        );
        console.log("Cleaned", waybill_items);

        let difference = data.filter((x) => !waybill_items.includes(x));

        console.log({ difference });
    }

    function selectRow(event) {
        let index = waybill_items.findIndex((x) => x.id == event.detail.id);
        waybill_items[index].selected = !event.detail.selected;
    }
    ...
    function selectAllRows(event) {
        ...
        waybill_items = event.detail.rows;
    }
    ...
    async function getStockReleases() {
        try {
            let params = {
                "pagination[limit]": -1,
                populate: ["stock_release_items", "stock_release_items.purchase_order_item" , "stock_release_items.purchase_order_item.item"],
            };
            ...
            console.log("Get Stock Releases ", response);
    ...
    function saveSelected() {
        let numberOfPurchaseOrders = waybill_items.filter(
            (x) => x.selected
        ).length;
                ...
                try {
                    let response = await post({
                        ...
                        data: {
                            data: {
                                stock_release_item:
                                    element.stock_release_item?.id,
                                waybill: element.waybill.id,
                    ...
                    console.log("Add Waybill Item Request  ", response);

                    if (response.data?.id) {
                        let index = waybill_items.findIndex(
                            (x) => x.id == element.id
                        );

                        waybill_items[index].status = "Done";
                        waybill_items[index].selected = false;
                    }
                } catch (e) {
                    let index = waybill_items.findIndex(
                        (x) => x.id == element.id
                    ...
                    waybill_items[index].status = "Error";
                    waybill_items[index].selected = false;
                    ...
                    waybill_items[index].message = message;
                    ...
                    console.log("Error Add Waybill Item  ", e);
                }

                if (index + 1 == numberOfPurchaseOrders) {
                    toast.push(
                        waybill_items.filter((x) => x.status == "Done").length +
                            " Waybill Item Added Successfully!",
                        {
                            duration: 20000,
                            theme: {
...
<div class="container px-6">
    ...
    <div class="card px-6">
        ...
        <div class="columns">
            <div class="column">
                <h3 class="my-0">Import Waybill Items</h3>
        ...
        {:else if waybill_items}
            <div class="has-text-centered">
                ...
                <p class="light-gray">No Waybill Items Found in the file</p>
                <br /><br /><br /><br />
            </div>
        {:else}
            <div class="has-text-centered">
