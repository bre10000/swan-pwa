<script>
        ...
        reader.onload = (e2) => {
            if (e.target.files[0].type == "text/csv") {
                ...
                let temp = utils.sheet_to_json(workbook.Sheets.Sheet1, {
                    raw: false,
                    header: ["category", "name", "unit", "pieces"],
                });
    ...
    function cleanData(data) {
        ...
        items = data.filter((x) => x.category && x.name && x.unit && x.pieces);
        console.log("Cleaned", items);
    }

    function selectRow(event) {
        let index = items.findIndex((x) => x.id == event.detail.id);
        items[index].selected = !event.detail.selected;
    }
    ...
    function selectAllRows(event) {
        ...
        items = event.detail.rows;
    }

    function saveSelected() {
        let numberOfItems = items.filter((x) => x.selected).length;
        ...
        items
            ...
            .forEach(async (element, index) => {
                try {
                    ...
                    if (response.data?.id) {
                        let index = items.findIndex((x) => x.id == element.id);
    ...
    <div class="card px-6">
        ...
        {:else if items}
            <div class="has-text-centered">
                ...
                <p class="light-gray">No Items Found in the file</p>
                <br /><br /><br /><br />
            </div>
        {:else}
            <div class="has-text-centered">
