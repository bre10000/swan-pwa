...
<script>
        ...
        try {
            ...
            formChildItems.forEach(async (element) => {
                if (element.createdAt) {
                    let response = await put({
                        path: "waybill-items/" + element.id + "?" + params,
                        data: {
                            data: {
                                waybill: waybill.id,
                                stock_release_item:
                    ...
                    console.log("Edit Waybill Items ", response);
    ...
    function removeChildItem(id, childItem) {
        ...
        formChildItems.forEach((element) => {
            getStockReleaseItems({ detail: element.stock_release }, element);
            getRemaining({ detail: element.stock_release_item }, element);
        });
    }
    ...
    async function getStockReleases(event) {
        try {
            ...
            formChildItems.forEach((element) => {
                ...
                //     (x) => x.id == element.stock_release.value
                // )[0];

                getStockReleaseItems(
                    { detail: element.stock_release },
                    element
                );
                getRemaining({ detail: element.stock_release_item }, element);
            });
    ...
    async function getRemaining(event, childItem) {
        if (event.detail?.value) {
            ...
            event.detail.data.attributes?.waybill_items?.data
                ?.filter(
                    (x) =>
                        x.attributes.waybill.data &&
                        formIDs.findIndex((y) => y == x.id) == -1
                )
                .forEach((element) => {
            ...
            formChildItems[index].waybill_items =
                event.detail.data.attributes?.waybill_items?.data?.filter(
                    (x) =>
                        x.attributes.waybill.data &&
                        formIDs.findIndex((y) => y == x.id) == -1
                );
        ...
        try {
            let params = {
                populate: [
                    ...
                    "waybill_items",
                    "waybill_items.stock_release_item",
                    "waybill_items.stock_release_item.waybill_items",
                    "waybill_items.stock_release_item.waybill_items.waybill",
                    "waybill_items.stock_release_item.waybill_items.waybill.consortium_member",
                    "waybill_items.stock_release_item.waybill_items.stock_release_item",
                    "waybill_items.stock_release_item.waybill_items.stock_release_item.stock_release",
                    "waybill_items.stock_release_item.waybill_items.stock_release_item.stock_release.consortium_member",
                    "waybill_items.stock_release_item.waybill_items.stock_release_item.purchase_order_item",
                    "waybill_items.stock_release_item.waybill_items.stock_release_item.purchase_order_item.item",
                    "waybill_items.stock_release_item.stock_release",
                    "waybill_items.stock_release_item.stock_release.stock_release_items",
                    "waybill_items.stock_release_item.stock_release.stock_release_items.purchase_order_item",
                    "waybill_items.stock_release_item.stock_release.stock_release_items.purchase_order_item.item",
                    "waybill_items.stock_release_item.purchase_order_item",
                    "waybill_items.stock_release_item.purchase_order_item.item",
                    "waybill_items.stock_release_item.purchase_order_item.purchase_order",
                ],
            ...
            driverTransportType.set(
                response.data.attributes.driverTransportType
            );
            ...
            formChildItems = response.data.attributes.waybill_items.data.map(
                (x) => {
                    return {
                        id: x.id,
                        stock_release: {
                            value: x.attributes.stock_release_item.data
                        ...
                        stock_release_item: {
                            value: x.attributes.stock_release_item.data.id,
                            label: x.attributes.stock_release_item?.data
                                ?.attributes.purchase_order_item.data.attributes
                                .item?.data.attributes.name,
                            data: x.attributes.stock_release_item.data,
                        },
                        ...
                        // stock_release_items: x.attributes.stock_release_item.data?.attributes.stock_release.data?.attributes.stock_release_items.data.map(x => {
                        //    return {
                        ...
                        unit: x.attributes.stock_release_item.data.attributes
                            .purchase_order_item.data?.attributes.item.data
                            .attributes.unit
                            ? x.attributes.stock_release_item.data.attributes
                                  .purchase_order_item.data?.attributes.item
                                  .data.attributes.unit
                            : "",
                        pieces: x.attributes.stock_release_item.data.attributes
                            .purchase_order_item.data?.attributes.item.data
                            .attributes.pieces
                            ? x.attributes.stock_release_item.data.attributes
                                  .purchase_order_item.data?.attributes.item
