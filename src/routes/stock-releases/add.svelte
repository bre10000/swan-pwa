...
import UnsavedConfirmation from "../../widgets/modals/UnsavedConfirmation.svelte";
    ...
    async function add() {
        ...
        try {
            ...
            toast.push(
                "Stock Release Could Not Be Added! \n\n" + e.error.message,
                {
                    duration: 20000,
                    theme: {
    ...
    async function getBatchNumbers(event, childItem) {
        try {
            ...
            formChildItems[index].stock_item = "";
            formChildItems[index].stock = "";
    ...
    async function getRemaining(event, childItem) {
        if (event.detail?.value) {
            ...
            formChildItems[index].unit =
                stock_item?.attributes.item?.data?.attributes.unit ? stock_item?.attributes.item?.data?.attributes.unit : "";
            formChildItems[index].pieces =
                stock_item?.attributes.item?.data?.attributes.pieces ? stock_item?.attributes.item?.data?.attributes.pieces : "";
